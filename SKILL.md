...
description: Create and publish a daily AI+HR intelligence brief for Xiaohongshu/Feishu communities. Use when Codex needs to collect same-day AI+HR and global AI news, enforce strict date/source verification, produce Chinese Feishu-ready Markdown or Feishu Docx content, or maintain an 08:00 Asia/Shanghai scheduled news workflow for HR, recruiters, and business owners.
---
...
Use the target date in Asia/Shanghai. The Feishu document title must be `YYYY.MM.DD`.
...
5. All date blocks and the final document title match the target date.
...
5. Keep the final human-facing summary concise: document title, accepted item counts, rejected item counts, Feishu URL if published, and any residual risk.
...
FEISHU_FOLDER_TOKEN=fldxxx \
python3 scripts/ai_hr_daily_brief.py --publish-feishu --output-dir output
```

## Scheduling

For GitHub Actions, use `.github/workflows/daily-ai-hr-brief.yml`. It runs at 08:00 Asia/Shanghai (`00:00 UTC`) and expects Feishu secrets to be configured in the GitHub repository.
